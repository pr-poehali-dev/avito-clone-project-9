...
import { AdminTab, SupportMessage, TICKETS, USERS, MODERATION_ADS, TRANSACTIONS } from "@/pages/Admin";

interface AdminTabsProps {
  tab: AdminTab;
  selectedTicket: number | null;
  replyText: string;
  ticketMessages: Record<number, SupportMessage[]>;
  moderFilter: string;
  platega: { key: string; connected: boolean; testMode: boolean };
  stats: { label: string; value: string; delta: string; icon: string; color: string }[];
  onSelectTicket: (id: number) => void;
  onReplyTextChange: (v: string) => void;
  onSendReply: () => void;
  onModerFilterChange: (v: string) => void;
  onPlatégaChange: (v: { key: string; connected: boolean; testMode: boolean }) => void;
}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              <div className="bg-gray-900 rounded-2xl border border-white/10 p-5">
                ...
                <div className="space-y-3">
                  {MODERATION_ADS.slice(0, 5).map(ad => (
                ...
                {selectedTicket ? (
                  <div className="bg-gray-900 rounded-2xl border border-white/10 flex flex-col h-[500px]">
                    ...
                    <div className="p-4 border-t border-white/10 flex gap-3">
                      ...
                      />
                      <button onClick={onSendReply} className="btn-gradient w-10 h-10 rounded-xl flex items-center justify-center shrink-0">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      value={platega.key}
                      onChange={e => onPlatégaChange({ ...platega, key: e.target.value })}
                      type="password"
                    ...
                    />
                    <button
                      onClick={() => onPlatégaChange({ ...platega, connected: !!platega.key })}
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl bg-gray-800">
                  ...
                  <button
                    onClick={() => onPlatégaChange({ ...platega, testMode: !platega.testMode })}
